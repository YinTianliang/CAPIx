...
void MemCopy(int argc, wchar_t *argv[])
{
	...
	else if(argv[2][0] == TYPE_PTR)
	{
		...
		if(argv[2][1] == TYPE_ASTR)
		{
			...
			//这段代码有bug,因为dst的大小可能没有达到8192,会溢出
			free(dst);
	...
	else if(argv[3][0] == TYPE_PTR)
	{
		...
		if(argv[3][1] == TYPE_ASTR)
		{
			...
			//这段代码有bug,因为src的大小可能没有达到8192,会溢出
			free(src);
...
bool CAPI(wchar_t *CmdLine)
{
	...
	{		//Mem
		...
		else if(!wcsicmp(argv[1], L"Put"))
		{	//Mem Put
			MemPut(argc, argv);
		}
		else if(!wcsicmp(argv[1], L"Print"))
		{	//Mem Print
			MemPrint(argc, argv);
		}
		else if(!wcsicmp(argv[1], L"Copy"))
		{	//Mem Copy
			MemCopy(argc, argv);
		}
	}
